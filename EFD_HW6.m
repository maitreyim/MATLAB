function [ optionPrices, St, middle] = EFD_HW6( type, kind, S0, K, r, sigma, T, delta, delta_t, Smax, Smin, logPrice )
%Funtion to valuate an option using Explicit Finite Difference Method.
%INPUT
...
%If we are going to use X = ln(S) or S to generate stock prices
if(logPrice == 1)
    [Xt, rows, middle] = StockPricesFiniteDifferenceMethod(log(S0), log(Smin), log(Smax), delta, 0);
    St = exp(Xt);%Change the prices from log
else
    [St, rows, middle] = StockPricesFiniteDifferenceMethod(S0, Smin, Smax, delta, 0);
end;

o = zeros(rows, columns); %Create array to capture option prices
%Option prices at maturity
if(kind == 'C')
    o(:,columns) = max(St - K, 0);
else
    o(:,columns) = max(K - St, 0);
end;
...
for i = columns-1:-1:1
    ...
    for j = 1:rows
        ...
        if(j == 1) %If first row in each column, we need to set bondaries
            if(kind == 'C') 
                o(j,i) = St(j)-K*exp(-r*(columns-i)*delta_t);
                EV = St(j) - K;
            else
                o(j,i) = 0;
                EV = K - St(j);
        ...
        elseif(j == rows) %Other bondaries for the last row in each column
            if(kind == 'C') 
                ...
                o(j,i) = K*exp(-r*(columns-i)*delta_t);
                EV = K - St(j);
            end;
        else %If not bondaries we calculate according to the explicit formula
            if(kind == 'C'); EV = St(j) - K;
            else EV = K - St(j); end;
            o(j,i) = (1/(1+r*delta_t))*(Pu*o(j-1,i+1)+Pm*o(j,i+1)+Pd*o(j+1,i+1));
        end;
        %If we are calculating for American option we need take Exercise
        %value into account
        if(type == 'A' && EV > o(j,i))
            o(j,i) = EV;
        end;
...
optionPrices = o; %return option prices in each node.
end
