...
%price = PriceDiscountBond(nominal, 0, 0, r0, 0, 0, sigma, 0, kappa,aveR, 0, 0, S, M, 'Vasicek')
% %%%%%%%%%%%%%PART 1 B%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % % %Get the price of a coupon paying bond using Vasicek
%price = PriceCouponBond(1000, 30, r0, sigma, kappa, aveR, 0, 4, M,0.5, 'Vasicek') 
% % % 
% % %%%%%%%%%%%PART1C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% P = PriceDiscountBond(nominal, 0, 0, ro(no), 0, 0, sigma, 0,kappa, aveR, 0, 0, S-T, M, 'Vasicek');
...
% P = PriceCouponBond(nominal, coupon, ro(no), sigma, kappa, aveR, T,4.0, M, 0.5, 'Vasicek');
% % % % % % % %     %Get the option price for each path
% oPrices(i) = exp(-mean(ro)*T)*max(P-K,0);
% end;
...
% P = ExplicitVasicekPriceCouponBond(nominal, coupon, ro(no), sigma,kappa, aveR, T, 4.0, 0.5);
% % % % % %     %Get the option price for each path
% oPrices(i) = exp(-mean(ro)*T)*max(P-K,0);
% end;
% option = mean(oPrices)%Get the average option price
...
% %%%%%%%%%%Question 2%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%QUESTION 2 A%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % %PARAMETER INITIALIZATION FOR QUESTION 2
...
% b = 0.3;
% rho = 0.7; S = 1; T = 0.5; no = ceil(T*365); Mo = 100; K = 2000;
% nominal = 2000; M = 100;
% % %%Number of paths to get the value of the option
% for i = 1:100
...
% % % %     %Get the price of a discount bond for each path
% P = PriceDiscountBond(nominal, x0, y0, ro(Mo), phi0, rho, sigma, nu, 0, 0,a, b, S-T, M, 'G2++');
...
% optionPrice = ExplicitG2PriceOption('P', 0.9, rho, sigma, nu, phi0, x0,y0, a, b, 0, T, S)
