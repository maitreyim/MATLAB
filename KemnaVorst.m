...
N = 100000;%No of simulation used in the study
OptionP = nan(27,8);
count = 1;
for i=1:3
for j=1:3
...
function [ Stock ] = StockPrices( S0, r, vol, T0, T, n, Q )
%GENERATION OF STOCK PRICES, PAPER USES A LOG PROCESS BUT HERE A MORE
%GENERAL APPROACH WAS TAKEN
Stock = nan(n+1,1);
Stock(1) = S0;
for i = 2:n+1
Stock(i,1) = Stock(i-1,1)*exp((r-0.5*vol^2)*(T-T0)/n+vol*sqrt((T-T0)/n)*Q(i-1));
end;
end
...
Arith = nan(N,1);%Pre-allocating memory space 
Geometric = nan(N,1);%Pre-allocating memory space 
Q = randn(n,N);% Generating random number to be used in the monte carlo simulation
for i = 1:N
Stock = StockPrices( S0, r, vol, T0, T, n, Q(:,i) );
Arith(i) = exp(-r*(T-T0))*max(mean(Stock)-K,0);
Geometric(i) = AverageOptionsecondMethod(S0, Stock, K, r, vol, T0, T, Arith(i));
end;
...
function [ ReducedVarValue] = AverageOptionsecondMethod( S0, Stock, K, r, vol, T0, T, Y )
...
W1= exp(-r*(T-T0));
geoW = W1*max(geomean(Stock)-K,0);
ReducedVarValue = exp(-r*(T-T0))*ExpW + (Y-geoW);%Reduced Average Option price
end
